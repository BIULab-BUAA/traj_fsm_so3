...
PX4CtrlFSM::PX4CtrlFSM(Parameter_t &param, LinearControl &controller): param_(param), controller_(controller)
{
    state_ = MANUAL_CTRL;
    hover_pose_.setZero();
}
...
void PX4CtrlFSM::process()
{
    ...
    {    /* code */
        ...
        else if(param_.takeoff_land.enable && takeoff_land_data_.triggered && takeoff_land_data_.takeoff_land_cmd == quadrotor_msgs::TakeoffLand::TAKEOFF)
        ...
        if(rc_data_.toggle_reboot)  // Try to reboot. EKF2 based PX4 FCU requires reboot when its state estimator goes wrong.
        {
            if(state_data_.current_state.armed)
            {
                ROS_ERROR("[px4ctrl] Reject armed! disarm the drone first");
            }
            reboot_FCU();
        }
        break;
    ...
    {
        // check state change
        if(!rc_data_.is_hover_mode || !odom_is_received(now_time))
        { // if not in hover, or no odom
            state_ = MANUAL_CTRL;
            toggle_offboard_mode(false);

            ROS_WARN("[px4ctrl] AUTO_HOVER(L2) --> MANUAL_CTRL(L1)");
        }
        else if(rc_data_.is_command_mode && cmd_is_received(now_time))
        {
            state_ = CMD_CTRL;
            des = get_cmd_des();
            ROS_INFO("\033[32m[px4ctrl] AUTO_HOVER(L2) --> CMD_CTRL(L3)\033[32m");
        }
        else if(takeoff_land_data_.triggered && takeoff_land_data_.takeoff_land_cmd == quadrotor_msgs::TakeoffLand::LAND)
        {
            state_ = AUTO_LAND;
            set_start_pose_for_takeoff_land(odom_data_);
    ...
    case CMD_CTRL:
    {
        ...
        if(takeoff_land_data_.triggered && takeoff_land_data_.takeoff_land_cmd == quadrotor_msgs::TakeoffLand::LAND)
        {
    ...
    case AUTO_LAND:
    {
        ...
        else if(!rc_data_.is_command_mode)
        {
            state_ = AUTO_HOVER;
            set_hov_with_odom();
            des = get_hover_des();
            ROS_WARN("[px4ctrl] From AUTO_LAND to AUTO_HOVER(L2)!");
        }
        else if(!get_landed())
        {
            des = get_takeoff_land_des(-param_.takeoff_land.speed);
...
void PX4CtrlFSM::land_detector(const State_t state, const Desired_State_t &des, const Odom_Data_t &odom)
{
    ...
    if(state == State_t::MANUAL_CTRL && !state_data_.current_state.armed)
