...
LastEditTime: 2024-02-05 06:31:47
...
if __name__ == '__main__':
    traj_pub = rospy.Publisher('/drone0/planning/pos_cmd', PositionCommand, queue_size=1)
    ...
    while not rospy.is_shutdown():
        ...
        if(t < 1e-6):
            continue
        # 8 zi
        traj.position.x =  4 * math.sin(t/4) 
        traj.velocity.x =  1 * math.cos(t/4)
        traj.acceleration.x = -0.25 * math.sin(t/4)

        traj.position.y =  4 * math.sin(t/2)
        traj.velocity.y = 2 * math.cos(t/2)
        traj.acceleration.y = -1 * math.sin(t/2)
        # traj.position.x = 0.8 * math.sin(0.2*t)
        # traj.velocity.x = 0.16 * math.cos(0.2*t)
        # traj.acceleration.x = -0.032 * math.sin(0.2*t)
        ...
        # traj.position.x =  4 * math.sin(t) 
        # traj.velocity.x =  4 * math.cos(t)
        # traj.acceleration.x = -4 * math.sin(t)

        # traj.position.y = 4 * math.cos(t)
        # traj.velocity.y = -4 * math.sin(t)
        # traj.acceleration.y = -4 * math.cos(t)
